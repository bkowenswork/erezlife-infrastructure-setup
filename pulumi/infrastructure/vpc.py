...
def create_vpc(vpcData, vpcRegion):
    ...
    igw = aws.ec2.InternetGateway(f"igw-{vpcData['vpcName']}", vpc_id=VPC.id, tags = tagged|{"Name":f"{vpcData['vpcName']} Internet Gateway"},opts=pulumi.ResourceOptions(provider=vpcRegion))
    ...
    for subnets in vpcData['subnets']:
        if subnets['type'] == "IGW":
            pub_subnet = aws.ec2.Subnet(f"{vpcData['vpcName']}-{subnets['name']}",vpc_id=VPC.id, cidr_block=subnets["cidr"], map_public_ip_on_launch=True, availability_zone=subnets['zone'], tags = tagged|{"Name":f"{vpcData['vpcName']} {subnets['name']}"},opts=pulumi.ResourceOptions(provider=vpcRegion))
            ...
            vpc.update({f"pubSubnet{public_nets}": pub_subnet.id})
    ...
    for subnets in vpcData['subnets']:
        if subnets['type'] == "NAT":
            private_subnet = aws.ec2.Subnet(f"{vpcData['vpcName']}-{subnets['name']}",vpc_id=VPC.id, cidr_block=subnets["cidr"], map_public_ip_on_launch=False, availability_zone=subnets['zone'], tags = tagged|{"Name":f"{vpcData['vpcName']} {subnets['name']}"},opts=pulumi.ResourceOptions(provider=vpcRegion))
...
def create_peering(requester, accepter, caRegion):
    ...
    vpc_peering_connection = aws.ec2.VpcPeeringConnection("vpcPeeringConnection",
        vpc_id=requester['vpcId'],                # ID of the requester VPC
        peer_vpc_id=accepter['vpcId'],            # ID of the VPC with which you are creating the VPC Peering Connection
